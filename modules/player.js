export default class Player {
  ...
  update(gameState, audio, timeElapsed) {
    const walls = gameState.walls;
    const pellets = gameState.pellets;
    const energizers = gameState.energizers;
    ...
    energizers.forEach((energizer, idx) => {
      if (this.playerCircleCollision(energizer)) {
        gameState.energizer(timeElapsed);
        energizers.splice(idx, 1);
      }
    });

    ghosts.forEach((ghost) => {
      if (this.playerGhostCollision(ghost)) {
        if (ghost.state === 'frightened') {
          ghost.position = {
            x: ghost.startingPosition.x,
            y: ghost.startingPosition.y,
          };
        } else {
          gameState.initializeMap();
        }
      }
    });
  ...
  playerGhostCollision(ghost) {
    if (
      this.position.y - this.radius <= ghost.position.y + ghost.radius &&
      this.position.x + this.radius >= ghost.position.x &&
      this.position.y + this.radius >= ghost.position.y &&
      this.position.x - this.radius <= ghost.position.x + ghost.radius
    ) {
      return true;
    }
  }
  ...
  playerCircleCollision(circle) {
    if (
      this.position.y - this.radius <= circle.y + circle.radius &&
  ...
  playerTileCollision(wall) {
    if (
      this.position.y - this.radius + this.velocity.y <= wall.y + wall.height &&
  ...
  drawPlayer(ctx) {
    ...
    this.animationCount =
      (this.animationCount + this.animationSpeed) % this.animationLoop.length;
  }
}
